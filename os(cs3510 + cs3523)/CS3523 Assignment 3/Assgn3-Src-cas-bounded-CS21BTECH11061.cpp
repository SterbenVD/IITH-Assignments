...
{
    srand(threadNo);
    string id = to_string(threadNo + 1);
    for (int i = 1; i <= cric_count; i++)
    {
        int nextThread = (threadNo + 1) % numThread;
        waiting[threadNo] = true;
        bool key = 1;
        bool expect = false;
        printfile(i, id, "Request");
        while (waiting[threadNo] == true && key == 1)
        {
            key = !lock.compare_exchange_strong(expect, newval, std::memory_order_relaxed);
            expect = false;
        }
        // Entry Section
        printfile(i, id, "Entry");
        csleep(mean1); // Simulation of critical - section
        while (nextThread != threadNo && waiting[nextThread] == false)
            nextThread = (nextThread + 1) % numThread;
        printfile(i, id, "Exit");
        if (nextThread == threadNo)
            lock.store(false);
        else
...
int main()
{
    ...
    remove(FILE_NAME);
    input.close();
    waiting = new bool[numThread];
    for (int i = 0; i < numThread; i++)
        waiting[i] = true;
    thread threadId[numThread];
    for (int i = 0; i < numThread; i++)
        threadId[i] = thread(testCS, i);
