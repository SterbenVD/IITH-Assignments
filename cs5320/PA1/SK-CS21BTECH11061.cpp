...
ofstream logs(LOGS_FILE);

// Singhal and Kshemkalyani optimization
...
private:
    ...
    int numProcesses;
    // LastSend[i] stores the time at which the last message was sent to process i
    vector<int> LastSent;
    // LastUpdate[i] stores the time at which vectorClocks[i] was last updated.
...
public:
    ...
    VC(int processId, int numProcesses, string file_name)
    {
        ...
        LastSent.resize(numProcesses, 0);
        LastUpdate.resize(numProcesses, 0);
        this->eventNo = 0;
    ...
    void increment()
    {
        ...
        LastUpdate[processId] = vectorClocks[processId];
    }
    ...
    void update(map<int, int> newVectorClocks)
    {
        for (auto x : newVectorClocks)
        {
            if (x.second > vectorClocks[x.first])
            {
                vectorClocks[x.first] = x.second;
                LastUpdate[x.first] = x.second;
            }
        }
    }
    ...
    string clock_send(int pairId)
    {
        ...
        for (int i = 0; i < numProcesses; i++)
        {
            if (pairId == -1 || LastUpdate[i] > LastSent[pairId])
            {
                s += to_string(i) + ":" + to_string(vectorClocks[i]) + ",";
                temp++;
            }
        }
        s.pop_back();
    ...
    void receive(string s)
    {
        ...
        while (i < s.size())
        {
            ...
            // string debug = id + "LOL" + rid + "LOL";
            // cout << debug << endl;
            if (id.empty() || rid.empty())
                break;
            newVectorClocks[stoi(id)] = stoi(rid);
            i++;
        }
        mtx.lock();
        increment();
    ...
    void main_func(EventType type, int pairId = -1, string msg = "")
    {
        ...
        if (type == SEND)
        {
            LastSent[pairId] = vectorClocks[processId];
        }
...
void process_send(int processId, VC &vc, vector<vector<int>> &adjMat)
{
    ...
    while (msg_count < numEvents)
    {
        ...
        else
        {
            ...
            socket.connect(addr);
            vc.clock_increment();
            zmq::message_t message = create_message(SEND, vc.clock_send(sendId), processId);
            socket.send(message);
